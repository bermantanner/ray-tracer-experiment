...
#include "./scenes/custom_scene.inl"
#include "Image.h"
...
void RayTracer::init(int scene_id) {
    ...
    if (scene_id == 1)
        ...
        scene = std::unique_ptr<Scene>(custom_scene());
    else {
...
Ray RayTracer::ray_thru_pixel(int i, int j) {
    ...
    /**
     * TODO: Task 1.1 (im modifying for my fish lens idea for Task 7)
     */
    ...
    float ndc_x = 2.0f * ((i + x) / static_cast<float>(camera.width)) - 1.0f;
    float ndc_y = 1.0f - 2.0f * ((j + y) / static_cast<float>(camera.height)); 
    ndc_x *= aspect;

    // actual distance from the center of the screen
    ...
    if (r > 1.0f) {
        ...
        float theta = r * max_angle; 

        float sin_theta = std::sin(theta);
        float cos_theta = std::cos(theta);

        // get the normalized 2D direction on the screen
        float dir_x = (true_r > 0.0f) ? (ndc_x / true_r) : 0.0f;
        float dir_y = (true_r > 0.0f) ? (ndc_y / true_r) : 0.0f;

        // map to the 3D spherical direction
        float x_comp = sin_theta * dir_x;
        float y_comp = sin_theta * dir_y;
        float z_comp = cos_theta;

        ray.dir = glm::normalize(x_comp * u + y_comp * v - z_comp * w);
    }
